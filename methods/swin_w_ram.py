...
class CrossMHAFusion(nn.Module):
    ...
    def forward(self, tgt, query_pos, sk, pos_sk):
        ...
        tgt2, attention = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), sk.transpose(0, 1))
        tgt2 = tgt2.transpose(0,1)
        
        tgt = self.forward_ffn(tgt, tgt2)

        return tgt, sk, attention, tgt2
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.stages_fuse = [0,1,2,3]

        # Sketch attention started
        ...
        attention_list.append(CrossMHAFusion(d_model=96))
        attention_list.append(CrossMHAFusion(d_model=192))
        attention_list.append(CrossMHAFusion(d_model=384))
        attention_list.append(CrossMHAFusion(d_model=768))

        position_list.append(PositionEmbeddingSine(96//2))
        position_list_sk.append(PositionEmbeddingSine(96//2))

        position_list.append(PositionEmbeddingSine(192//2))
        position_list_sk.append(PositionEmbeddingSine(192//2))

        position_list.append(PositionEmbeddingSine(384//2))
        position_list_sk.append(PositionEmbeddingSine(384//2))

        position_list.append(PositionEmbeddingSine(768//2))
        position_list_sk.append(PositionEmbeddingSine(768//2))

        self.position_list = nn.ModuleList(position_list)
        self.position_list_sk = nn.ModuleList(position_list_sk)

        attention_proj = nn.Sequential(
              # This is 1x1 conv -> so linear layer
              nn.Conv2d(256, 96, kernel_size=1),
              nn.GroupNorm(32, 96),
            )
        attention_proj_list.append(attention_proj)
        attention_proj = nn.Sequential(
              # This is 1x1 conv -> so linear layer
        ...
        normalize_image.append(nn.GroupNorm(32, 96))
        normalize_sketch.append(nn.GroupNorm(32, 96))

        normalize_image.append(nn.GroupNorm(32, 192))
        normalize_sketch.append(nn.GroupNorm(32, 192))

        normalize_image.append(nn.GroupNorm(32, 384))
        normalize_sketch.append(nn.GroupNorm(32, 384))

        normalize_image.append(nn.GroupNorm(32, 768))
        normalize_sketch.append(nn.GroupNorm(32, 768))
        projection = []
        projection.append(nn.Conv2d(in_channels=2*96, out_channels=96, kernel_size=1, stride=1, padding=0, bias=False))
        projection.append(nn.Conv2d(in_channels=2*192, out_channels=192, kernel_size=1, stride=1, padding=0, bias=False))
        projection.append(nn.Conv2d(in_channels=2*384, out_channels=384, kernel_size=1, stride=1, padding=0, bias=False))
        projection.append(nn.Conv2d(in_channels=2*768, out_channels=768, kernel_size=1, stride=1, padding=0, bias=False))

        self.attention = nn.ModuleList(attention_list)
        self.attention_proj = nn.ModuleList(attention_proj_list)
        self.normalize_image = nn.ModuleList(normalize_image)
        self.normalize_sketch = nn.ModuleList(normalize_sketch)
        self.projection = nn.ModuleList(projection)
        self.op1 = nn.Conv2d(384, 192, 1)

        self.op3 = nn.Conv2d(192, 192, 1)

        self.bn_op = nn.GroupNorm(32, 192)


        self.resConfUnit1 = ResidualConvUnit(192, torch.nn.ReLU(), bn=True)
        self.resConfUnit2 = ResidualConvUnit(192, torch.nn.ReLU(), bn=True)
        self.skip_add = nn.quantized.FloatFunctional()
        # Sketch attention complete
    ...
    def forward(self, x, mask, sketches):
        """ Forward function.
        ...
        for stage in range(self.num_layers):
            ...
            # compute the attention map for each block  for fusion sketch and patch
            if stage in self.stages_fuse:
                x = x.permute(0,2,1)
                x = x.view(x.shape[0], x.shape[1], Wh, Ww)
