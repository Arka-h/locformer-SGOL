...
# 6,100,000 samples / 10,000 per shard = 610 shards
# TOTAL_SHARDS=610

# 61 array tasks × 10 shards each = 610 shards
# SHARDS_PER_TASK=10
...
. ./.env
                            # --shard_start "$START" \
                            # --shard_end "$END" \
PYTHONUNBUFFERED=1 python convert_data.py   --qd_root $COCO_HOME/processed_quickdraw/processed_quick_draw \
                                            --out_lmdb $COCO_HOME/processed_quickdraw/lmdb/qd_all.lmdb\
                                            --resume