#!/bin/bash
#SBATCH --job-name=lf_ow_rn50                 # Job name
#SBATCH --output=outputs/lf_ow_rn50_%j.log    # Standard output log (%j = job ID)
#SBATCH --error=outputs/lf_ow_rn50_%j.err     # Standard error log
...
# =============================================================
# Open-world run: train on the 42 seen COCO categories, evaluate per-epoch on
# the 14 held-out (i%4==0) unseen categories using one-shot sketch queries.
...
OUT_DIR="$PROJECT_HOME/outputs/lf_ow_rn50"
mkdir -p "$OUT_DIR"
RESUME_ARG=""
